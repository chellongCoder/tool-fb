...
//   };
  ...
  timeout = async () => {
    return await new Promise(resolve => this.getData(5, resolve));
  }
  ...
  getData = async (limit?: number, resolve?: any, ) => {
    await axios({
      ...
      console.log(`🛠 LOG: 🚀 --> -------------------------------------------------------------------`);
      console.log(`🛠 LOG: 🚀 --> ~ file: app.ts ~ line 74 ~ App ~ getData= ~ res`, res.data.__fb_trace_id__, res.data.data);
      console.log(`🛠 LOG: 🚀 --> -------------------------------------------------------------------`);
      const records: any[] = [];
      const next: string = res.data.paging.next;
      this.next = next;
      // for {limit}} thằng lấy ra data page 
      await Promise.all(res.data.data.slice(0, limit).map(async (element: any, index:number) => {
      ...
      console.log('end', res.data.__fb_trace_id__);
      resolve(next);
      // lưu data vào file 
      // fs.writeFile("output.json", JSON.stringify(res.data.__fb_trace_id__), 'utf8', function (err) {
      //   if (err) {
      ...
      // });
      // fs.appendFile('output.json', `${JSON.stringify(records)}\n`, function (err) {
      //   if (err) {
      //     console.log("An error occured while writing JSON Object to File.");
      //     return console.log(err);
      // }
  ...
  run() {
    this.app.listen(this.port, async () => {
      ...
      //     console.log("Your public (through Tor) IP is: " + JSON.parse(body));
      //   }
      // });
